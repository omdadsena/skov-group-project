          ...
          {submitted ? (
            ...
            <form onSubmit={submit} className="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <input required className="input-dark" value={form.name} onChange={(e) => update("name", e.target.value)} />
              </div>
              <div>
                <label className="label-gold">Phone *</label>
                <input required inputMode="numeric" maxLength={10} className="input-dark" value={form.phone} onChange={(e) => update("phone", e.target.value.replace(/\D/g, ""))} />
              </div>
              <div>
                <label className="label-gold">Email</label>
                <input type="email" className="input-dark" value={form.email} onChange={(e) => update("email", e.target.value)} />
              </div>
              <div>
                <label className="label-gold">City *</label>
                <input required className="input-dark" value={form.city} onChange={(e) => update("city", e.target.value)} />
              </div>
              <div className="sm:col-span-2">
                <label className="label-gold">Service needed *</label>
                <select className="input-dark" value={form.serviceType} onChange={(e) => update("serviceType", e.target.value)}>
              ...
              <div className="sm:col-span-2">
                <label className="label-gold">Project details</label>
                <textarea rows={5} className="input-dark resize-none" value={form.message} onChange={(e) => update("message", e.target.value)} placeholder="Plot size, budget, timeline, and what you need help with…" />
              </div>
              {error && <p className="sm:col-span-2 rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-300">{error}</p>}
